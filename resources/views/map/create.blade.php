        ...
        `;
            ...
            streetInput.addEventListener('input', debounce(async (e) => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        if (response.status >= 500) {
                            console.error('Server error in street search:', response.status);
                            streetSuggestions.innerHTML = '<li class="error">Сервис геокодирования перегружен. Попробуйте ещё раз.</li>';
                        } else if (response.status === 404) {
                            streetSuggestions.innerHTML = '<li class="error">Улицы не найдены</li>';
                        } else {
                            streetSuggestions.innerHTML = '<li class="error">Ошибка загрузки. Проверьте запрос.</li>';
                        }
                    ...
                    if (!contentType || !contentType.includes('application/json')) {
                        const text = await response.text();
                        console.error('Expected JSON, but got:', text.substring(0, 200));
                        streetSuggestions.innerHTML = '<li class="error">Ошибка формата данных.</li>';
                        streetSuggestions.classList.add('visible');
                        return;
                    }
                    ...
                    if (!Array.isArray(streets)) {
                        ...
                        streetSuggestions.innerHTML = '<li class="error">Ошибка данных с сервера.</li>';
                        streetSuggestions.classList.add('visible');
                        return;
                    }
                    ...
                    if (error.name === 'TypeError' && error.message.includes('failed to fetch')) {
                        streetSuggestions.innerHTML = '<li class="error">Нет соединения. Проверьте интернет.</li>';
                    } else {
                        streetSuggestions.innerHTML = '<li class="error">Ошибка загрузки улиц. Попробуйте позже.</li>';
                    }
                    streetSuggestions.classList.add('visible');
                }
            }, 300));
